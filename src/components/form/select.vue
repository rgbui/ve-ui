<template>
  ...
  >
    ...
    <div v-if="spread" class="vp-select-dropbox" :style="selectDropStyle">
      <vp-select-options :options="cacOptions"></vp-select-options>
    </div>
  </div>
</template>
<script lang="ts">
...
export default Vue.extend({
  ...
  computed: {
    currentOption() {
      var opsList = this.cacOptions;
      if (Array.isArray(opsList)) {
        if (this.multiple == true) {
          var ops = opsList.arrayJsonFindAll("childs", (x) => {
            return Array.isArray(this.value) && this.value.includes(x.value);
          });
          return ops;
        } else {
          var op = opsList.arrayJsonFind(
            "childs",
    ...
    cacOptions() {
      if (this.emptyOption) {
        if (this.options) return [this.emptyOption, ...this.options];
        else return [this.emptyOption];
  ...
  methods: {
    loadAsyncOptions(force?: boolean) {
      if (force == true) this.loadAsyncOptionsed = false;
      if (typeof this.asyncOptions == "function") {
        if (this.loadAsyncOptionsed == true) {
          if (
            !(
              Array.isArray(this.options) &&
              this.cachedAsyncOptions &&
              this.cachedAsyncOptions.length == this.options.length
            )
          ) {
            if (this.cachedAsyncOptions)
              this.$set(this, "options", this.cachedAsyncOptions);
          }
          return;
        }
        this.loadAsyncOptionsed = true;
        this.asyncOptions().then((r) => {
          this.cachedAsyncOptions = r;
          this.$set(this, "options", r);
        });
      }
    },
    remove(index) {
