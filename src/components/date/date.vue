...
export default Vue.extend({
  ...
  watch: {
    value(newValue, oldValue) {
      if (newValue instanceof Date) {
        ...
        Object.assign(this, {
          year: year,
          day: (newValue as Date).getDate(),
          month: (newValue as Date).getMonth() + 1,
          hour: (newValue as Date).getHours(),
          minute: (newValue as Date).getMinutes(),
          second: (newValue as Date).getSeconds(),
          fromYear:
            Math.floor(year % 10 == 0 ? (year - 1) / 10 : year / 10) * 10,
          toYear: Math.ceil(year / 10) * 10,
        });
        if (this.dateText) this.showDateText();
      } else if (typeof newValue == "string") {
        var date = moment(newValue).toDate();
  ...
  data() {
    ...
    if (typeof this.value == "string") {
      this.$set(this, "value", new Date(this.value));
    }
  ...
  methods: {
    ...
    setEmit(force?: boolean) {
      ...
      if (this.format == "string") {
        date = `${date.getFullYear()}-${this.pad(
          date.getMonth() + 1
        )}-${this.pad(date.getDate())} ${this.pad(date.getHours())}:${this.pad(
          date.getMinutes()
        )}:${this.pad(date.getSeconds())}` as any;
      }
      if (!this.dateText) this.showDateText();
